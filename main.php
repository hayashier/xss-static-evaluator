...
**/
	...
	include('suggestion/tokens.php');			// tokens for lexical analysis
	include('suggestion/printer.php');				// output scan result
	include('suggestion/util.php');
	include('suggestion/score.php');
...
<?php
	// scan result
	$result = @printoutput($output, $_POST['treestyle']);
	//echo 'Dump: ';dump($result,false,true);
	$scores = array();
	$total = 0;
	foreach($result as $key => $value) {
		$obj = $value[0];
		$scores[number_format($obj->score,1)]++;
		$total++;
	}
	echo "Score: ";dump($scores,false,true);
