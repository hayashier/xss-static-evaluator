...
**/
	...
	function printoutput($output, $treestyle=1)
	{
		if(!empty($output))
		{
			...
			$freq = array();
			do {
				foreach($temp[key($temp)] as $vulnBlock)
				{
					$freq = calcFrequency($vulnBlock);
				}
			}
			while(next($temp));
			...
			do
			{
				if(key($output) != "" && !empty($output[key($output)]) && fileHasVulns($output[key($output)]))
				{
					...
					foreach($output[key($output)] as $key => $vulnBlock)
					{
						//EDIT
						$score = judgeScore($vulnBlock);
						$digit = 2;
						$output[key($output)][$key]->score = $score;
						//ENDEDIT
						if($vulnBlock->vuln)
						{
							$nr++;
							...
							' onClick="hide(\'',$vulnBlock->category,$nr,'\')">';

							//TODO: Show score
							echo '</div><div class="score">'.number_format($score, $digit).' point</div>';

							echo '<div class="vulnblocktitle">',$vulnBlock->category,'</div>',
							...
							foreach($vulnBlock->treenodes as $tree)
							{
								...
								// if(empty($tree->funcdepend) || $tree->foundcallee )
								{
									...
									if(isset($GLOBALS['scan_functions'][$tree->name]))
									{
										...
										//TODO: Show score
										echo "<div class='score'>'.number_format($score, $digit).' point</div>\n";
